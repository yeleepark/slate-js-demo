...
- 기본 베이스 경로는 `/slate-js-demo`로 설정되어 있으므로 `https://<username>.github.io/slate-js-demo/`에 바로 배포할 수 있습니다. 다른 경로로 배포하려면 `NEXT_PUBLIC_REPO_BASE=<다른-경로>`를 지정한 뒤 빌드하세요.
...
4. 다른 리포지토리 이름이나 커스텀 경로에 배포하고 싶다면 빌드 전에 `NEXT_PUBLIC_REPO_BASE` 환경 변수를 원하는 경로로 설정하세요.
